...
const AppointmentsPage = () => {
  ...
  const handleMoveAppointmentDepartment = async (appointmentId, doctorDeptName) => {
    if (!window.confirm(`Move this appointment to ${doctorDeptName} department?`)) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/patient/${patientId}/appointments/${appointmentId}/move-department`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        }
      ...
      setAppointments(appointments.map(apt =>
        apt.appointment_id === appointmentId
          ? {
            ...apt,
            appointment_department_id: apt.doctor_department_id,
            appointment_department_name: apt.doctor_department_name
          }
          : apt
      ));

      alert('✅ Appointment moved to your doctor\'s department!');
    } catch (err) {
                      ...
                      >
                        <div className="flex flex-col lg:flex-row gap-8">
                          ...
                          <div className="flex-1">
                            ...
                            {/* Department */}
                            {appointment.appointment_department_name && (
                              <div className="mb-6">
                                <div className="flex items-center gap-3 mb-2">
                            ...
                            {appointment.appointment_department_id !== appointment.doctor_department_id && (
                              <div className="mb-6 p-4 rounded-lg bg-red-50 border-l-4 border-red-400">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    <p className="text-sm font-bold" style={{ color: '#7F1D1D' }}>Department Mismatch Alert</p>
                                    <p className="text-xs mt-2" style={{ color: '#991B1B' }}>
                                      This appointment is scheduled in <strong>{appointment.appointment_department_name}</strong> but your doctor is assigned to <strong>{appointment.doctor_department_name}</strong> department.
                                    </p>
                                    <button
                                      onClick={() => handleMoveAppointmentDepartment(appointment.appointment_id, appointment.doctor_department_name)}
                                      disabled={movingDept === appointment.appointment_id}
                                      className="mt-3 px-3 py-1.5 rounded-lg text-xs font-semibold text-white transition-all"
                                      style={{
                                        backgroundColor: '#DC2626',
                                        opacity: movingDept === appointment.appointment_id ? 0.6 : 1,
                                      }}
                                    >
                                      {movingDept === appointment.appointment_id ? 'Moving...' : `Move to ${appointment.doctor_department_name}`}
                                    </button>
                                  </div>
                                </div>
                              </div>
